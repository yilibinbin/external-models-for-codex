...
export function classifyProviderFailure(result = {}, options = {}) {
  ...
  const outcomeKind = String(result.outcome?.kind || options.outcomeKind || "");
  const text = textOf(result.stderr, result.error?.message || result.error, result.outcome?.message, options.diagnostic, options.message);
  if (String(options.category || "")) {
    return normalizeFailureCategory(options.category);
  }
  if (outcomeKind === "success" || (status === 0 && stdout && !errorCode && !text)) {
    return "";
  }
  if (/capacity_blocked|maximum active|resource lease|global.*capacity/i.test(text)) {
    return "capacity_blocked";
  }
  if (outcomeKind === "timeout" || errorCode === "ETIMEDOUT" || /timed?\s*out|timeout|ETIMEDOUT/i.test(text)) {
    return "timeout";
  }
  if (outcomeKind === "auth" || /not logged in|not logged into|unauthenticated|authentication failed|login required|invalid api key/i.test(text)) {
    return "auth";
  }
  if (/rate[_ -]?limit|too many requests|\b429\b/i.test(text)) {
    return "rate_limit";
  }
  if (outcomeKind === "quota" || /quota|RESOURCE_EXHAUSTED/i.test(text)) {
    return "quota";
  }
  ...
  if (outcomeKind === "invalid-stream" || /Invalid stream|invalid_stream|malformed tool call|empty response/i.test(text)) {
    return "invalid_stream";
  }
  if (outcomeKind === "malformed-output" || /malformed|invalid json|did not contain a JSON object|JSON output|parse/i.test(text)) {
    return "malformed_json";
  }
  if (/model.*(not available|unavailable|not found)|unknown model|unsupported model/i.test(text)) {
    return "model_unavailable";
  }
  if (outcomeKind === "permission" || /permission|unsupported flag|provider compatibility|not supported/i.test(text)) {
    return "provider_compatibility";
  }
  if (outcomeKind === "empty-output" || (status === 0 && !stdout)) {
    return "empty_output";
  }
  return "unknown";
}
