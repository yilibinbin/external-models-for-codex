...
function runReleaseCheck(rawArgs = []) {
  ...
  try {
    ...
    if (manifest.version !== "0.11.1") failures.push(`manifest version is ${manifest.version}, expected 0.11.1`);
...
function checkGithubActionsCi() {
  ...
  try {
    ...
    if (!plain.includes("--ref gemini-for-codex-v0.11.1")) failures.push("github actions missing immutable Gemini release ref");
