...
function assertSafeTasksetId(id) {
  ...
  if (!SAFE_TASKSET_ID.test(text)) {
    throw new Error("Taskset id must match ts-[A-Za-z0-9_-]+ and contain no path separators.");
  }
  return text;
}
...
function tasksetFile(cwd, id, env) {
  ...
  if (relative.startsWith("..") || path.isAbsolute(relative)) {
    throw new Error("Taskset path escaped state directory.");
  }
  return file;
}
...
export function normalizeTaskset(input) {
  ...
  return {
    ...
    title: String(input.title || "Claude taskset").trim(),
