...
function longRunningLifecycleChecks(pluginRoot) {
  ...
  return [
    ...
    result(jobs.includes("Running job cancellation did not deliver a signal") && jobs.includes("signalDelivered") && jobs.includes("preserveActive"), "cancel-requires-delivered-signal", "running cancel cannot report cancelled unless a validated signal was delivered"),
    ...
    result(jobs.includes("const cancelledByRequest = Boolean(current.cancelRequestedAt);"), "cancel-request-finish-semantics", "worker results after a persisted cancel request are treated as cancelled"),
    ...
    result(jobs.includes("lockOwnerMatches") && jobs.includes("command: process.argv.join") && jobs.includes("identity.command === owner.command") && jobs.includes("writeError") && jobs.includes("fs.rmSync(lockFile"), "owner-aware-file-locks", "stale lock cleanup checks owner identity and cleans up metadata write failures"),
