...
function longRunningLifecycleChecks(pluginRoot) {
  ...
  return [
    ...
    result(companion.includes("function reserveBackgroundJob") && companion.includes("reserveJob(cwd") && companion.includes("findActiveJobByIdempotencyKey") && companion.includes("canStartBackgroundJob") && companion.includes("capacity_blocked"), "reserve-job-cap-idempotency", "host-forwarded reserved jobs use the workspace cap and idempotency path"),
    ...
    result(jobs.includes("Running job cancellation did not deliver a signal") && jobs.includes("signalDelivered") && jobs.includes("preserveActive") && jobs.includes("Job reached a terminal state before cancellation failure could be persisted"), "cancel-requires-delivered-signal", "running cancel cannot report cancelled unless a validated signal was delivered"),
