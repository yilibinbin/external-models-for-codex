...
function longRunningLifecycleChecks(pluginRoot) {
  ...
  return [
    ...
    result(lifecycle.includes("abandonedReservation") && jobs.includes("reservation-expired") && jobs.includes("Host-forwarded reserved job was not claimed") && jobs.includes("mutateLostJobUnderLock") && jobs.includes('current.status !== "queued"'), "queued-reservation-expiry", "abandoned host-forwarded reserved jobs release capacity after queued timeout only after locked revalidation"),
