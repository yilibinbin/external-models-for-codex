...
function longRunningLifecycleChecks(pluginRoot) {
  ...
  return [
    ...
    result(companion.includes("workingTreeFingerprintMatches") && companion.includes("workingTreeFingerprintDetails(cwd, [], hookFingerprintOptions())") && unread.includes("hookFingerprintOptions") && unread.includes("workingTreeFingerprint(cwd, [], hookFingerprintOptions())") && fingerprint.includes("legacyHashes") && fingerprint.includes("hashStdoutParts") && fingerprint.includes("HOOK_GIT_SIGNAL_TIMEOUT_MS") && fingerprint.includes("HOOK_MAX_UNTRACKED_FINGERPRINT_BYTES"), "review-gate-baseline-shared-fingerprint", "review-gate and UserPromptSubmit hook share bounded fingerprint logic with legacy baseline compatibility"),
    result(companion.includes("diffFingerprint.timedOut") && companion.includes("diffFingerprint.budgetExceeded") && companion.includes("working-tree fingerprint timed out; allowing stop") && companion.includes("running review without cached gate decision") && fingerprint.includes("budgetExceeded: parts.some"), "review-gate-fingerprint-timeout-fail-open", "review-gate fails open for true fingerprint timeouts but still reviews when only the bounded untracked fingerprint budget is exceeded"),
    result(/function hasReviewableGitChanges[\s\S]{0,700}gitShort\(\["rev-parse"/.test(companion) && /function hasReviewableGitChanges[\s\S]{0,900}gitShort\(\["status"/.test(companion) && companion.includes('reason: "git status timed out"') && companion.includes("reviewable.reason}; allowing stop") && companion.includes('args.quality = "standard"') && companion.includes("gitRunner: (gitArgs) => gitShort(gitArgs, cwd)"), "review-gate-reviewable-git-timeout", "review-gate bounds reviewable git probes, quality policy, and prompt git context before running Claude"),
    ...
    result(companion.includes("CLAUDE_FOR_CODEX_GIT_SIGNAL_TIMEOUT_MS") && companion.includes("ETIMEDOUT") && companion.includes("git signal collection timed out") && companion.includes("status.timedOut") && companion.includes("staged.timedOut") && companion.includes("unstaged.timedOut") && companion.includes("branch?.timedOut") && companion.includes("branchNames?.timedOut"), "git-timeout-not-nonrepo", "git timeout is distinct from not-a-repository and is detected through the run() helper's errorCode"),
