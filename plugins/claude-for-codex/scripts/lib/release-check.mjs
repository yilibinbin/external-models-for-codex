...
function longRunningLifecycleChecks(pluginRoot) {
  ...
  return [
    ...
    result(companion.includes("CLAUDE_FOR_CODEX_KILL_GRACE_MS") && companion.includes("SIGKILL") && companion.includes("processGroupHasLiveMembers") && companion.includes('stopChildGroup("SIGKILL", { requireLiveGroup: true })'), "hard-timeout-sigkill", "hard timeout escalates after grace period only when the process group still has live members"),
