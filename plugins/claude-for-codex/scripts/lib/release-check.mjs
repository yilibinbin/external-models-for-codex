...
function longRunningLifecycleChecks(pluginRoot) {
  ...
  return [
    ...
    result(processText.includes("DEFAULT_PS_TIMEOUT_MS") && processText.includes("CLAUDE_FOR_CODEX_PS_TIMEOUT_MS") && processText.includes("killSignal: \"SIGKILL\"") && processText.includes("maxBuffer: psProbeMaxBufferBytes()") && /function processGroupHasLiveMembers[\s\S]{0,500}return true;/.test(processText), "process-ps-probe-bounds", "process validation ps probes are bounded and inconclusive process-group scans preserve capacity"),
