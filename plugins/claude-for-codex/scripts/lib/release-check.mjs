...
function longRunningLifecycleChecks(pluginRoot) {
  ...
  return [
    ...
    result(companion.includes("function reserveBackgroundJob") && companion.includes("reserveJob(cwd") && companion.includes("findActiveJobByIdempotencyKey") && companion.includes("canStartBackgroundJob") && companion.includes("capacity_blocked") && companion.includes("alreadyRunning") && companion.includes("do not dispatch a forwarding subagent"), "reserve-job-cap-idempotency", "host-forwarded reserved jobs use the workspace cap and idempotency path without returning invalid worker commands for direct jobs"),
    ...
    result(companion.includes("workingTreeFingerprintDetails") && companion.includes("fingerprint.timedOut") && companion.includes("fingerprintTimedOut") && fingerprint.includes("INCONCLUSIVE"), "job-idempotency-timeout-no-reuse", "background idempotency is disabled when worktree fingerprint collection times out or fails"),
    ...
    result(/function hasReviewableGitChanges[\s\S]{0,700}gitShort\(\["rev-parse"/.test(companion) && /function hasReviewableGitChanges[\s\S]{0,900}gitShort\(\["status"/.test(companion) && companion.includes('reason: "git status timed out"') && companion.includes("reviewable.reason}; allowing stop"), "review-gate-reviewable-git-timeout", "review-gate bounds reviewable git probes before running Claude"),
    ...
    result(lifecycle.includes("abandonedReservation") && jobs.includes("reservation-expired") && jobs.includes("Host-forwarded reserved job was not claimed"), "queued-reservation-expiry", "abandoned host-forwarded reserved jobs release capacity after queued timeout"),
    ...
    result(jobs.includes("Running job cancellation did not deliver a signal") && jobs.includes("signalDelivered") && jobs.includes("missingStartingChildSupervision") && jobs.includes("before child supervision metadata was persisted") && jobs.includes("preserveActive") && jobs.includes("Job reached a terminal state before cancellation failure could be persisted"), "cancel-requires-delivered-signal", "running cancel cannot report cancelled unless a validated signal was delivered and startup child supervision is known"),
    ...
    result(processText.includes("export function processGroupHasLiveMembers") && processText.includes("\"-eo\"") && !processText.includes("\"-axo\"") && processText.includes("leaderless") && jobs.includes("leaderless-orphaned"), "leaderless-child-group-cleanup", "child process groups can be preserved and cleaned after the leader exits"),
