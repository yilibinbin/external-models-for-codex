...
const DEFAULT_RELEASE_REF = "claude-for-codex-v0.10.0";
...
const MARKER = "<!-- claude-for-codex-review -->";
...
function readTemplate(pluginRoot) {
  return fs.readFileSync(path.join(pluginRoot, "templates", "github-actions", "claude-for-codex-review.yml"), "utf8");
}
...
function parseRoles(value) {
  ...
  for (const role of roles) {
    if (!VALID_ROLES.has(role)) {
      throw new Error(`Invalid GitHub Actions review role "${role}".`);
    }
  }
  return roles;
}
...
export function renderWorkflow(pluginRoot, options = {}) {
  ...
  const multiReview = Boolean(options.multiReview);
  const roles = parseRoles(options.roles ?? "correctness,security,tests");
  const semanticContext = options.semanticContext ?? "off";
  if (semanticContext === "auto") {
    throw new Error("--semantic-context auto is not supported for CI workflows; use off or an explicit provider.");
  }
  ...
  const annotationStep = annotations ? [
    ...
    "          script: |",
    "            const fs = require('fs');",
    "            const annotations = JSON.parse(fs.readFileSync('claude-for-codex-annotations.json', 'utf8'));",
  ...
  return readTemplate(pluginRoot)
    ...
    .replaceAll("{{MODEL}}", escapeYamlString(options.model ?? ""))
    .replaceAll("{{EFFORT}}", escapeYamlString(options.effort ?? ""))
...
export function extractRunBlocks(text) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    for (let cursor = index + 1; cursor < lines.length; cursor += 1) {
      ...
      if (current.trim() && current.match(/^(\s*)/)[1].length <= baseIndent) {
        break;
      }
      block.push(current);
    }
...
export function validateWorkflow(text, options = {}) {
  ...
  const checks = [
    ...
    result(body.includes("Claude review skipped for fork pull request"), "github-actions-fork-safe"),
...
export function renderReviewComment(review) {
  ...
  if (!findings.length) {
    ...
    for (const finding of findings) {
      ...
      lines.push(`  ${sanitizeText(finding.recommendation ?? finding.message ?? "")}`);
    }
  }
...
export function reviewToAnnotations(review) {
  ...
  const annotations = [];
  for (const finding of findings) {
    if (!validAnnotationPath(finding.file)) {
      continue;
    }
    const line = Number.isInteger(finding.line) && finding.line > 0 ? finding.line : 1;
    const endLine = Number.isInteger(finding.end_line) && finding.end_line >= line ? finding.end_line : line;
    annotations.push({
      path: finding.file,
      start_line: line,
      end_line: endLine,
      annotation_level: annotationLevel(finding.severity),
      title: sanitizeText(finding.title ?? "Claude for Codex finding"),
      message: sanitizeText(finding.recommendation ?? finding.message ?? finding.summary ?? "")
    });
  }
  return annotations;
}
