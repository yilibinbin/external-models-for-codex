...
不可变的 `claude-for-codex-v0.14.0` ref 用于从这个多插件 marketplace 安装 Claude 插件切片。Gemini 应使用自己的 release ref 安装，或仅在开发时从 `main` 安装。
...
Claude 原生 SDK 模式必须显式开启。使用 `multi-review --backend sdk --agent-team sdk-subagents` 时，运行时会为所选审阅角色创建 Claude SDK native subagents。SDK 包优先解析 `@anthropic-ai/claude-agent-sdk`，同时保留 `@anthropic-ai/claude-code` 作为兼容 fallback。加上 `--native-structured` 会请求 SDK schema-backed 聚合输出；加上 `--stream-progress` 会输出脱敏后的流式进度，但不会打印原始 SDK chunk，也不会把原始 SDK message 写入报告。
