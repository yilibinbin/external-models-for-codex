...
- Claude for Codex：让 Codex 调用本地 Claude Code CLI，获得独立审阅、scorecard 审阅、taskset 规划、validation evidence 审阅、有界 assisted-review、多角色复审、原生 SDK subagent 团队、救援诊断、结构化审阅输出、需要费用确认的 ultrareview 和可选 Stop Hook 门禁；同时包含 dynamic model alias registry、request-local outcome classification、全局 Claude 进程 governor、低成本 `doctor --json` 诊断和 fork-safe CI dogfood。
- Gemini for Codex：让 Codex 调用 legacy Gemini CLI（`gemini`）做 Gemini-only 只读审阅、scorecard 审阅、taskset 规划、计划文件审阅、有界 assisted-review、规划、救援诊断、结构化审阅输出、角色包、后台任务和 Gemini CLI 原生 session 能力探测。
...
- `claude-plan-review`：让 Claude 审阅已保存的实施计划文件。Claude SDK 原生 subagents 需要显式选择 `--backend sdk --agent-team sdk-subagents`；ultrareview 仍然只在用户明确提到并确认费用时使用。
- `claude-assisted-review`：运行显式有界 scorecard 反馈；Claude 只审阅，Codex 判断并修复。
...
Claude for Codex 0.20.1 新增建议性的质量反馈能力：`review --scorecard --json`、`multi-review --scorecard --json`、`plan --taskset`、`review --validation-log <file>`、`assisted-review --scorecard --max-review-rounds 2` 和 `result --resume-plan <job-id>`。Scorecards are advisory，validation logs are user-provided；Claude 仍不会自动编辑文件、运行项目命令、提交、推送、创建 PR、合并、关闭 issue，也不会自动调用 ultrareview，除非用户显式使用独立的 ultrareview skill。
...
## 全局 Claude 进程 Governor

Claude for Codex 会用全局资源 governor 限制所有插件拥有的 Claude work slots，包括前台 review、后台 jobs、插件管理的多角色并行 fan-out、SDK subagent 审阅团队、Stop hooks 和 Codex subagent delegation。

默认锁目录位于仓库外的 `~/.codex/claude-for-codex/global-resource-locks`。如果需要迁移，可设置 `CLAUDE_FOR_CODEX_GLOBAL_RESOURCE_LOCK_DIR=<absolute-private-dir>`。该目录必须是绝对路径、当前用户拥有、非 symlink、非 group/world writable 的私有目录；不安全根目录会被拒绝，而不是静默使用。

用 `CLAUDE_FOR_CODEX_MAX_CLAUDE_PROCESSES=<n>` 调整全局 Claude work-slot 上限。默认值按主机检测到的可用并行度自适应并设置小上限；常见 5 核以上主机会保留常规五角色并行审阅能力，低资源主机会自动降低并发。`0` 会阻止新的 Claude 启动，适合作为紧急逃生或测试设置。

容量耗尽时，插件返回 `capacity_blocked`，不会继续无限启动 Claude work。JSON 前台命令会输出 `{"status":"capacity_blocked", ...}` 并以 `75` 退出；普通前台命令会在 stderr 输出简短原因并以 `75` 退出；后台 job 会进入终态 `capacity_blocked`；Stop hook 会 fail open 并输出警告。如果插件管理的并行角色审阅还有部分容量但不足以覆盖所有角色，会自动降级为 sequential，并在报告中记录 `executionMode: "sequential"`。SDK subagent 团队最多按请求角色数预留 slot，但会受当前主机 effective limit 封顶；如果整个 native-agent 团队无法安全进入容量预算，会返回 `capacity_blocked`。

资源 lease 会在 Claude work 运行期间刷新。如果命令有已知的长超时，运行时会把该 lease 的 TTL 下限提升到至少覆盖操作超时加刷新余量；如果过期 lease 的 owner PID 仍然存活，或 liveness 无法确定，插件会保留该 lease，而不是直接回收。

`setup`、`status`、`doctor --json` 和脱敏报告会暴露 `resourceGovernor`、`capacityStatus`、`lockRootClass`、requested slots、effective max 和 available slots，但不会泄露原始锁目录路径。
