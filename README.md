...
Claude for Codex 0.20.0 keeps those aliases in a shared model alias registry used by CLI policy and SDK subagent selection. It also records compact outcome classification metadata for CLI/SDK runs, exposes `doctor --json` as a cheap no-prompt health check, documents SDK subagents as fresh isolated context reviewers, ships fork-safe repository CI validation, and hardens SDK timeout handling across permission-compatibility retries.
